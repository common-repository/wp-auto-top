        ...
        <form method="post">
            <table class="form-table" >
                ...
                <tr>
                    <td>显示位置</td>
                    <td>
                      <select name="autotop_position" style="width:100px;" >
                        <option value="left"<?php if($autotop_position == 'left') echo 'selected="selected"';?>>左</option>
                        <option value="right" <?php if($autotop_position == 'right') echo 'selected="selected"';?>>右</option>
                      </select> 相对网页正中间线距离:<input type="number" name="autotop_margin"  style="width:60px;" value="<?php echo $autotop_margin;?>" />
                      <select name="autotop_margin_unit">
                        <option value="px" <?php if($autotop_margin_unit == 'px') echo 'selected'; ?>>px</option>
                        <option value="%" <?php if($autotop_margin_unit == '%') echo 'selected'; ?>>%</option>
                      </select>, 距离顶部:<input type="number" style="width:60px;" name="autotop_top_val" value="<?php echo $autotop_top_val; ?>" />
                      <select name="autotop_top_unit">
                        <option value="px" <?php if($autotop_top_unit == 'px') echo 'selected'; ?>>px</option>
                        <option value="%" <?php if($autotop_top_unit == '%') echo 'selected'; ?>>%</option>
                      </select>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <td>文章评论位置标记</td>
                    <td><select name="autotop_comment_id_type">
                      <option value="id">id</option>
                      <option value="class">class</option>
                      <option value="name">name</option>
                    </select> = <input type="text" name="autotop_comment_id" value="<?php echo $autotop_comment_id ?>" /></td>
                </tr>
                <tr>
                    <td>鼠标悬停时滚动速度</td>
                    <td><input type="number" name="autotop_scroll_speed" value="<?php echo $autotop_scroll_speed ?>" /> * 越大越快</td>
                </tr>
                <tr>
                    <td>自动滚动</td>
                    <td>
                       <select name="autotop_scroll">
                        <option value="1" <?php if($autotop_scroll == '1') echo 'selected'; ?>>是</option>
                        <option value="0" <?php if($autotop_scroll == '0') echo 'selected'; ?>>否</option>
                      </select>
                    </td>
                </tr>
                <tr>
...
';
   ...
   echo sprintf(
    '<script> var commentPositionId = "%s";var wpAutoTopSpeed = %s; var wpAutoTopScroll = %s; </script>',
    $commentPositionId,  get_option('autotop_scroll_speed', 1), get_option('autotop_scroll', 1)
  );
